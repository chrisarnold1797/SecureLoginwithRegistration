...
The program is set up to be compiled on either windows or linux. For some reason, when using g++ to compile it on linux, the compiler will have a hard time locating definitions for bcrypt::validatePassword() and bcrypt::generateHash() functions. One way to avoid this compiler error is to make sure all the files are in the same directory and enter this in the cli:
g++ -g -Wall -v -include bcrypt.cpp -include blowfish.cpp main.cpp
